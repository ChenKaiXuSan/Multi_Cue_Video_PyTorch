...
def train(hparams, fold:int):
    ...
    # for the tensorboard
    tb_logger = pl_loggers.TensorBoardLogger(save_dir=os.path.join(hparams.train.log_path), version=fold)
...
def init_params(config, ):
    ...
    config.train.version = '_'.join([DATE, str(clip_duration), str(uniform_temporal_subsample_num)])
    
    # output log to file 
    log_path = '/workspace/Multi_Cue_Video_PyTorch/logs/' + '_'.join([config.train.version, config.model.model]) + '.log'
