...
class DefineCrossValidation(object):
    ...
    def make_dataset_with_video(self, val_dataset_idx: list, fold: int, flag: str):
        ...
        raw_video_path = glob.glob(
            "/workspace/data/segmentation_dataset_512/fold0/**/**/*.mp4"
        ...
        for path in val_idx:
            ...
            video_name = path.stem.split(".")[0]  # get the video name without extension

            for file in raw_video_path:
                if video_name in file:
                    video_path = Path(file)
                    break

            video_disease = path.parent.name  # get the disease name from the path
